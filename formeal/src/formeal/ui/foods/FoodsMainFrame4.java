...
public class FoodsMainFrame4 extends JFrame {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					FoodsMainFrame4 frame = new FoodsMainFrame4();
...
//				(Color) Color.orange));
		...
		+ "</html>";
			...
			//사진을 담은 레이블과 버튼 두개를 받아볼 패널
			JPanel panelInBtn = new JPanel();
			
			panelInBtn.setBorder(new MatteBorder(3, 2, 1, 1, 
					(Color) Color.orange));
			//panelInBtn.setSize(width, height);
			panelInBtn.setBackground(new Color(0, 255, 0, 255));
			panelInBtn.setOpaque(false);
			
			panelInBtn.setLayout(new GridLayout(1,3));
//			
			ImageIcon icon = new 
				ImageIcon(foods.getImagePath());
				
			
			JButton btnBasket= new JButton("레시피 보기");
//			btnBasket.setSize(10, 10);
			JButton btnRecipe= new JButton("재료 사기");
//			btnRecipe.setSize(10, 10);
			JLabel lbFoodMenu = 
					new JLabel(icon, JLabel.CENTER);
			
			lbFoodMenu.setToolTipText(foods.getfoodName() + " 주문하기");
			...
			panelInBtn.add(lbFoodMenu);
			panelInBtn.add(btnBasket);
			panelInBtn.add(btnRecipe);
			panel.add(panelInBtn);
