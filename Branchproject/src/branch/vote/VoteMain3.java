...
//			else party = ETC_PARTY;
		...
		// 득표수 누적 집계 D, H, E
		if(party == DONALD_PARTY)
			voteCountDonald++; // 도날드당 득표 1+
		else if(party == HILARY_PARTY)
			voteCountHilary++; // 힐러리당 득표 1+
		else
			voteCountEtc++; // 군소 후보들 득표 1+
			...
			System.out.printf("\t득표상황: Donald %d표 vs Hilary %d표 (기타: %d표\r\n",
					voteCountDonald, voteCountHilary, voteCountEtc);
			vorder++;
		} // loop exit
