...
//			int menu = sc.nextInt();
		...
		for (int i = 0; i < menuOrders.length; i++) {
			...
			String mType = (i == 0) ? "메인" : "사이드";
			int mOrder = menuOrders[i]-1;
			String mName = (i == 0) ? strMainMenu[mOrder] : strSideMenu[mOrder];
			int mPrice = (i == 0) ? nMainPrices[mOrder] : nSidePrices[mOrder];
			System.out.println(mType + "메뉴: " + (mOrder+1) + ")" + mName + "\t -" + mPrice + "원");
			totalPrice += mPrice;
		}
		System.out.println("-----------------------------------");
		System.out.printf("\t 총 결제 금액: %5d원 \n", totalPrice);
		System.out.println("-----------------------------------");
	}
	// 함수 오버로딩 (Overloading)
