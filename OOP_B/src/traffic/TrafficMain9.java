...
//		System.out.println(vh instanceof Vehicle ? "비클입니다" : "아닙니다");
	...
	public static void displayVehicleInfo(Vehicle vh) {
		System.out.println("** VEHICLE TYPE:: " + vh.type + Vehicle.TYPES[vh.type] + " 바퀴: " + vh.wheel);
		
//		System.out.println(vh instanceof Vehicle ? "비클입니다" : "아닙니다");
		if(vh instanceof Car) { // type == Car_type
			Car car = (Car)vh;
			System.out.println("\t Car::"
					+ "타이어: " + car.tire + "구동방식: " + car.IMWD[car.immobility]);
			if(vh instanceof Van) {
				Van van = (Van)vh;
				System.out.println("\t\t Van::"
						+ " 간이쉼터: " + van.box);
			}else if(vh instanceof CampingCar) {
				CampingCar camp = (CampingCar)vh;
				System.out.println("\t\t CampingCar::"
						+ "간이쉼터: " + camp.bed);
			}else if(vh instanceof SportsCar) {
				SportsCar sport = (SportsCar)vh;
				System.out.println("\t\t SportsCar::"
						+ "가속도: " + sport.accel);
				if(vh instanceof AISportsCar) {
					AISportsCar ai = (AISportsCar)vh;
					ai.accelerateCar(100.0);
					System.out.println("\t\t AISportsCar"
							+ "ai타입: " + ai.aiType);
			...
			ByCycle by = (ByCycle)vh;
			System.out.println("\t ByCycle::"
					+ ", 페달: " + by.pedal);
			if(vh instanceof UniCycle) {
				System.out.println("\t\t");
				UniCycle uni = (UniCycle)vh;
				System.out.println(uni.toString());
			}else if(vh instanceof MotoBike) {
				System.out.println("\t\t");
				MotoBike mb = (MotoBike)vh;
				System.out.println(mb);
				if(vh instanceof Tukshai) {
					System.out.println("\t\t\t");
					Tukshai tt = (Tukshai)vh;
					System.out.println(tt.toString());
				}
			}
		}else if(vh instanceof Truck) {
			Truck tr = (Truck)vh;
			System.out.println("\t Truck::" + ", 적재량: " + tr.volume + ", 적재하중: " + tr.weight);
		}else {
			System.out.println("아직 분류가 안되는 2세대 비클?" + vh);
			// 비행기, 배, 지하철, 탱크, 드론???
		}
	}
...
//		printVehicle(bc2);
		...
		UniCycle bc3 = new UniCycle();
		
		MotoBike bc4 = new MotoBike();
		
		Tukshai bc5 = new Tukshai();
...
//		printVehicle(vn1);
		...
		int v = 1;
		SpaceShip sps1 = new SpaceShip();
		sps1.brand = "테슬라 드래건A";
		SpaceShip sps2 = new SpaceShip();
		sps2.brand = "아마존 블루오리진B";
		Vehicle[] vhs = { cr1, sc1, cc1, vn1, new AISportsCar("네이버클로버2식AI"),
				bc1, bc2, bc3, bc4, bc5, tk1, sps1, sps2 };
			for (Vehicle vh : vhs) {
				System.out.print("#" + v++ + " ");
				displayVehicleInfo(vh);
			}	
...
class SpaceShip extends Vehicle {
	...
	public String toString() {
		return "\t\t\t\t\t\t----" +  rocket + "을 이용한" +  this.brand + "탈것이 우주로 날아갑니다";
	}
