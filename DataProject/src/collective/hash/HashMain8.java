...
//		HashMap<String, NeoHero> heroes = null;
		...
		// 키: 팀이름문자열   =>  밸류: 영웅리스트
		HashMap<String, ArrayList<NeoHero>>
					teamMap = new HashMap<>();
		teamMap.put(NeoHero.DC_TEAM, 
				new ArrayList<NeoHero>()); // 팀원 0명 리스트 생성 
		teamMap.put(NeoHero.MARVEL_TEAM, 
				new ArrayList<NeoHero>()); 
		teamMap.put("재욱팀", new ArrayList<NeoHero>());
		
		//
		heroList.add(new NeoHero("재욱", "재욱팀"));
		heroList.add(new NeoHero("규진", "재욱팀"));
		heroList.add(new NeoHero("종세", "재욱팀"));
		heroList.add(new NeoHero("동현", "재욱팀"));
		//
		for (int i = 0; i < heroList.size(); i++) {
			NeoHero hero = heroList.get(i);
			// i번째 영웅 객체의 소속팀을 확인 
			if (hero.getTeam().equals(NeoHero.DC_TEAM)) {
				ArrayList<NeoHero> dcTeam = // 요소 리스트
					teamMap.get(NeoHero.DC_TEAM);
				dcTeam.add(hero);
			} else if(hero.getTeam().equals(NeoHero.MARVEL_TEAM)) {
				teamMap.get(NeoHero.MARVEL_TEAM).add(hero);
		...
		for (String teamName : teamNameSet) {
			// 팀 이름 키에 대응하는 팀맵의 요소객체가 바로 영웅들의 리스트
			ArrayList<NeoHero> heroTeam
						= teamMap.get(teamName);
			System.out.println(">> team: " + teamName 
						+ " , 팀소속 영웅들: " 
						+ heroTeam.size() +"명");
			for (NeoHero nh : heroTeam) {
				System.out.println("\t\t hero:: " + nh);
			}
			System.out.println("~~~~~~");
		}
	}
